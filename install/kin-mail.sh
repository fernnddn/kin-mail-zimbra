...
pick_one_stage() {
  ...
  while true; do
    ...
    printf '  %s%s%s  %s\n' "$BLD" "8)" "$RST" "05-healthcheck.sh        ${DIM}acceptance tests${RST}"
    printf '  %s%s%s  %s\n' "$BLD" "9)" "$RST" "00-config.sh --reset     ${DIM}re-run configuration wizard${RST}"
    printf '  %s%s%s  %s\n' "$BLD" "0)" "$RST" "Back"
    hr
    printf '  %sChoice%s [0-9]: ' "$BLD" "$RST"
    ...
    case "$choice" in
      ...
      8) run_stage 05-healthcheck.sh; return $? ;;
      9)
        echo
