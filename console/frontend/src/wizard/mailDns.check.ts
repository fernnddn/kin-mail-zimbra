...
export function assertMailDnsContract(): void {
  ...
  eq(isPublicIPv4("10.0.0.3"), false, "rfc1918");
  ...
  eq(spfRecord("10.0.0.3", ""), "v=spf1 mx ~all", "spf private ignored");
  eq(aRecordValue("10.0.0.3", "", ""), "", "no private A");
  eq(aRecordValue("203.0.113.10", "203.0.113.11", ""), "203.0.113.10", "A prefers http");
  eq(aRecordValue("10.0.0.3", "203.0.113.11", ""), "203.0.113.11", "A falls back to smtp");
  eq(aRecordValue("203.0.113.10", "203.0.113.11", "203.0.113.9"), "203.0.113.9", "A prefers public VIP");
  eq(aRecordValue("203.0.113.10", "", "10.0.0.99"), "203.0.113.10", "private VIP ignored");
